 ...
 *******************************
 */
unsigned char RXByteFIFOindex = 0;//Used by RXByteFIFO array while receiving messages
unsigned char TXByteFIFOindex = 0;//Used by TXByteFIFO array when filling out the response to a message
...
//TODO: Each device needs a unique address
...
const unsigned short crc8Table[]=
...
{
	unsigned int* par[1];
	initRS485(MAIN_COMPUTER,par);
	int ad=0;
	unsigned char send[]={'B','0','P','I','N','G','0','0','0','0'};
	addressChar='A';
	unsigned char crc[2];
	charToASCIIHex(generateCRC8(send,10),crc);
	...
	UART1PutChar(crc[0]);
	UART1PutChar(crc[1]);
	UART1PutChar(EOL_CHAR);
	testVar=0;
	PR1 = RS_485_TEST_PC_WAIT_RESPONSE;
...
void initRS485(char deviceType,unsigned int* data[])
{
	...
	TRISA = (0xFFF5 & TRISA);  // Set RA3 and RA1 pins as output, and RA0 as input
	LATA = 0;                   // Set initial state for all outputs
	LATB = 0;
...
void RS485processRX()
{
    ...
    rs485tempIndex=0;
	if(	LATA & 0x0001 || errorDisplay) // check RA0 pin for error detection
	{
		if(errorLEDPattern[errorDisplay]==errorCount)
		{
		errorDisplay++;//Increment stage of error 
		LATA=((~LATA) & 0x0002) | LATA; //set RA1 pin to opposite value
		}
		else
			errorCount++;
		if(errorDisplay==9)
		{
			errorDisplay=0;
			LATA=LATA & 0xFFFD; //set RA1 pin to low output
		}
 	}
...
//TODO: Testing on marked code
				...
				case(RS_485_EVAL_CHECKSUM):
				{
					...
					if(crcSent==generateCRC8(RXByteFIFO,rs485tempFIFOindex-3))
					{
			...
			case(RS_485_POST_CMD_DELAY):
			{
				...
				if (rs485postCmdDlyDone)
				{
					...
					LATA=LATA|0x0008;//Set FX pin(RA3) to high
...
{ 
	...
	if(!receiving)
	{
		...
		if(receivedChar==addressChar)//Dont start filling the recieve array until the first character of myAdress is received
			receiving=1;
	}
...
//		}
	...
	if (rs485state <= RS_485_POST_CMD_DELAY && testVar)
	{
		/*
...
//Commented out for transceiver testing
        IEC0bits.T1IE = 0;
		/*
