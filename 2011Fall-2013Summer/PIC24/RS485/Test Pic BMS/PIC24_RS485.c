...
char RXByteFIFO[RS_485_RX_BYTE_FIFO_SIZE];
char TXByteFIFO[RS_485_TX_BYTE_FIFO_SIZE];
char RXdata[RS_485_MAX_RX_DATA_BYTES];

//Data arrays to be filled with pointers to either BMS or MPPT variables and malloced to the appropriate size
int* MPPTStatus[RS_485_MPPT_STATUS_LENGTH];
int* BMSStatus[RS_485_BMS_STATUS_LENGTH];
#define BMS 0
/*******************************
 * Global variables
 *******************************
...
void initRS485(char deviceType,unsigned int* data[], unsigned char address[])
{
	...
	break;
	  ...
	  TRISB	 =	TRISB& 0xEFFF;	 //configure all PortB as input
	AD1CON1 = 0b00001111;         // RA0,1,2,...  digital	
	OSCCON = 0b01110000;         // Use Fosc = 8 MHz (Fcpu = 4 MHz) 
	LATB=LATB&0xEFFF;//Set RB12 pin to low
...
if (!IEC0bits.U1RXIE)
	{
			...
			case(RS_485_POST_CMD_DELAY):
			{
				...
				if (rs485postCmdDlyDone){
					...
					LATB=LATB | 0x1000;//Set FX pin(RB12) to high
