...
//Data arrays to be filled with pointers to either BMS or MPPT variables 
...
//CRC table used to compute the CRC for each message
...
/*
* Has to be called in MPPT/BMS code before main loop
* deviceType is either MPPT/BMS (defined value in h file)
* data has to have pointers to MPPT/BMS variables as specified in the RS485 spec sheet
* address has two chars which indicate the assigned address of the device
*
* Addresses will have M for mppt pics and B for BMS pics for the first character, then 1-9 as second char
*/
void initRS485(char deviceType,unsigned int* data[], unsigned char address[])
{
	...
	//Oscillator set up should be done in MPPT/BMS code
	//OSCCON = 0b01110000;        
	AD1CON1 = AD1CON1|0b00001111;         // RA0,1,2,...  digital
	TRISA = (TRISA&0xFFF5);  // Set RA3 and RA1 to output
	LATA = 0x0002;// Set RA1 to high to power the trasciever  (may be removed later)
...
//Simply sends out the given char through UART Tx
...
/*
*Method that controls the state of communication - once message is received it parses and responds as appropriate
*/
void RS485processRX()
{
    ...
    rs485tempIndex=0;
		...
		//Reset state if message timed out while receiving
		if (rs485intraMsgTimedOut &&  !(RXByteFIFO[RXByteFIFOindex-1] == EOL_CHAR) && (rs485state <= RS_485_LISTEN_EOL)) 
		{
			rs485resetState();
		}
...
if (!IEC0bits.U1RXIE)
	{
			...
			switch (rs485state)
				...
				case(RS_485_EVAL_CHECKSUM):
				{
					...
					if(crcSent==generateCRC8(RXByteFIFO,rs485tempFIFOindex-2))
					{
...
/*
*Given the four letter command code, verifies if such a command code is valid
*/
...
/*
*Fills TX buffer with appropriate content based on the message received.
*/
void rs485PopulateTXByteFIFO()
{
	...
	{
		//Always start message with own address
...
void rs485resetState()
{
		...
		rs485intraMsgTimedOut = 0;
		IEC0bits.U1RXIE = 1;
		LATA=LATA&0xFFF7;//Set RA3 pin to low
		rs485postCmdDlyDone = 0;
...
/*Deprecated variables no longer in use but saved because of usage in other old code
...
/*Deprecated functions not used in algorithm
