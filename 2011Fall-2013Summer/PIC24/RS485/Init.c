...
void initUART()
{
		U1MODE=0;
		U1STA=0;
	        AD1PCFGL = 0xFFFF;//Done to enable UART receive functionality on configurable pins-can be limited to pin being used
  	//-------------------------configure UART ---------------------------------------------------------//
		U1MODEbits.BRGH=1;//High Speed Baud Rate (Fosc/ 4)
		U1MODEbits.PDSEL1=0;//8 bits, no parity
		U1MODEbits.PDSEL0=0;
		U1MODEbits.STSEL=0;//1 stop bit
		//TODO: complete with appropriate value for the oscillation rate of the using pic
 		U1BRG=x;//x=Fcy/(4*115200) - example:Fosc= 8Mhz;Fcy=Fosc/2=4Mhz ; Baud Rate 115200 (4000000)/(4*115200)-1=7
...
//Not currently used in code due to not beling able to get it to work properly
/*
 void initCRC()
{
