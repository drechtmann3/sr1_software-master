...
//Global variables that need to be checked in BMS/MPPT code to receive commands
extern unsigned char MPPT_On;//Always inverse of MPPT_Off
extern unsigned char MPPT_Off;
extern unsigned char BMS_On;//Always inverse of BMS_Off
extern unsigned char BMS_Off;
extern unsigned char MPPT_Low;//Will be 0 if MPLO command not sent, otherwise will be the new efficiency to operate at

//Defines for device types. BMS/MPPT groups need to send correct defined value to initRS485
#define BMS (0)
#define MPPT (1)
...
#define RS_485_RX_BYTE_FIFO_SIZE (15) // Max PC message length - MPLO
#define RS_485_TX_BYTE_FIFO_SIZE (185) // Max message length for a response by BMS
...
#define RS_485_INTRA_MSG_TIME_OUT_H (0x1B8)	// Time needed to receive each byte
#define RS_485_INTRA_MSG_TIME_OUT_L (0x80)  // at 115200 baud
#define RS_485_POST_CMD_DELAY_H (0xFE)		//
