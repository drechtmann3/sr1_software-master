...
---------------------------------------------------------------------------*/
#define  WORDS_IN_FLASH_BUFFER 0x5               // Programming data buffer, Words
...
SECTOR Sector[8]= {
         (Uint16 *) 0x3D8000,(Uint16 *) 0x3DBFFF, // This is Flash Sector H, which we store data in.
...
/**
 * \brief Initializes flash clock and unlocks flash, should be called before flash is read.
 */
void initializeFlashAPI() {
// First, make sure we are at the right clock frequency. I don't trust Ricky's code, so make sure the clock is OK.
// The clock is very important for Flash!
// We set the clock frequency to 90MHz
...
/**
 * \brief Saves SOC into flash with 32-bit precision.
 * \return 0 if there is a flash error writing
 * \return 1 if flash written OK.
 */

#pragma CODE_SECTION(Example_CallFlashAPI,"ramfuncs");
float32 savedSOCInFlash;

char writeSOCIntoFlash()
{
	//return 1;
	Uint16  Status;
...
------------------------------------------------------------------*/
   ...
   if(VersionHex != 0x0100)
   {
	   ...
	   puts("Version incorrect!");
	   return 0;
       //asm("    ESTOP0");

   }   
   puts("Flash version OK");
...
------------------------------------------------------------------*/
   ...
   // Erase only Sector H, which stores our data.
   Status = Flash_Erase((SECTORH),&FlashStatus);
   if(Status != STATUS_SUCCESS) 
   {
       puts("Flash error erasing!");
	   return 0;
...
// then nothing is done and an error is returned. 
    ...
    // Example: Program 0x400 values in Flash SectorH
    SOC32Bit = (Uint32)(SoC / 51.2f * 1e6f);
    Buffer[0] = SOC32Bit & 0xFFFFFFFFL; // Save bottom 16 bits
    Buffer[1] = SOC32Bit>>16; // Save top 16 bits
    Buffer[2] = 3251; // My magic number, for verification.
    
    Flash_ptr = Sector[0].StartAddr; // Flash sector H
    Length = 3; // 3 words.
...
puts("programmed flash memory!");
    ...
    if(Status != STATUS_SUCCESS) 
    {
        puts("Could not verify flash written!");
        return 0;
    } else {
    	puts("Flash data verified!");
    	//savedSOCInFlash = SoC;
    	return 1;
    }
} 
...
/**
 * \brief Reads SOC from flash and saves into SOC variable.
 * \return 0 if there is a data integrity error.
 */
char readSOCFromFlash() {
	Uint32 SOCVal;
	// Read Flash:
	if (*(Sector[0].StartAddr + 2) != 3251) {
		puts("Flash data read invalid!");
		return 0;
	}
	SOCVal = ((Uint32)(*(Sector[0].StartAddr + 1))<<16) | *(Sector[0].StartAddr);
	//SoC = ((float32)SOCVal) / 1e6f * 51.2f;
	//savedSOCInFlash = SoC;
	return 1;
}
